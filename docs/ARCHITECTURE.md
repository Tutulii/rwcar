...
7. The optional agent plane exposes only semantic MCP/REST actions. The API records a hash-bound intent after mandate and live preflight checks; a separately deployed Privy executor rechecks policy/preflight and signs exact steps. Administrators choose autonomous or supervised delegation and always retain pause, permanent revocation, and mandate replacement authority. See [AGENT_PLATFORM.md](AGENT_PLATFORM.md).
