...
Read `delegation.executionMode` before writes. `AUTONOMOUS` means the administrator already delegated every listed `allowedAction` for the mandate lifetime and limits; do not request another approval. `SUPERVISED` may return `APPROVAL_REQUIRED`.
...
3. Inspect accrued interest, early-break fee, protocol fee, the automatically composed aUSDC approval when needed, and claim-based settlement details.
4. Execute and wait for `COMPLETED`.
5. The lender reads `settlementClaims[].prepareClaimInput` from `get_portfolio`, adds a fresh idempotency key, calls `prepare_claim`, executes it, and confirms completion.
...
3. A seller, lender, or third-party agent whose mandate includes `START_AUCTION` may also call `prepare_position_action`. Do not provide or invent a valuation ID; RWCAR resolves the current signed valuation server-side. Autonomous mode needs no per-intent approval; supervised mode may require one.
...
Manual auction starts, auction purchases, and liquidation-risk actions obey the signed execution mode. They run without another prompt under an autonomous mandate and require review under a supervised mandate. Autonomous keeper execution remains a separate permissionless service path.
...
5. Read collateral, debt, LTV, oracle freshness, margin-call deadline, and exposures before every later action. Autonomous mandates execute granted margin actions without per-leg human approval.
...
## Supervised approval handoff

Only when a supervised intent reports `APPROVAL_REQUIRED`, provide the administrator:
