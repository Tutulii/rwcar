...
const indexer = new RepoIndexer(config, db);
...
const keeper = config.KEEPER_PRIVATE_KEY ? new DefaultKeeper(config, db) : null;
...
if (!keeper) console.warn('Lifecycle keeper disabled because KEEPER_PRIVATE_KEY is not configured.');
if (!v2Indexer) console.warn('V2 indexer disabled because V2_DEPLOYMENTS_JSON is empty.');
await Promise.all([indexer.run(), v2Indexer?.run(), keeper?.run(), v2Automation?.run()]);
